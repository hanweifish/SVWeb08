...
function Racer(name) {
  this.name = name;
  this.stopAndWatch = new stopAndWatch();
  Racer.all.push(this);
}
Racer.all = [];
...
Racer.getWinners = function() {
  ...
  for (var i = 0; i < Racer.all.length; i++) {
    if (Racer.all[i].log() < found.log()) found = Racer.all[i];
  }
  return found;
};
Racer.prototype.start = function() {
  this.stopAndWatch.start();
  return this;
};
Racer.prototype.stop = function() {
  this.stopAndWatch.stop();
  return this;
};
Racer.prototype.log = function() {
  return this.stopAndWatch.log();
};
